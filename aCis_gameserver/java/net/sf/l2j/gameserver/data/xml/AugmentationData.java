...
public class AugmentationData implements IXmlReader
{
	...
	public Augmentation generateRandomAugmentation(int lifeStoneLevel, int lifeStoneGrade)
	{
		...
		if (generateSkill)
		{
			...
			{
				case 1 -> _blueSkills.get(lifeStoneLevel).get(Rnd.get(0, _blueSkills.get(lifeStoneLevel).size() - 1)); // blue skill
				case 2 -> _purpleSkills.get(lifeStoneLevel).get(Rnd.get(0, _purpleSkills.get(lifeStoneLevel).size() - 1)); // purple skill
				case 3 -> _redSkills.get(lifeStoneLevel).get(Rnd.get(0, _redSkills.get(lifeStoneLevel).size() - 1)); // red skill
				default -> stat34;
			};
