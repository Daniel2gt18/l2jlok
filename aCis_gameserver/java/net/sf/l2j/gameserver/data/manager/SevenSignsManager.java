...
public class SevenSignsManager
{
	...
	private boolean isNextPeriodChangeInPast()
	{
		...
		switch (_activePeriod)
		{
			...
			case RECRUITING, RESULTS -> lastPeriodChange.setTimeInMillis(_lastSave.getTimeInMillis() + PERIOD_MINOR_LENGTH);
		}
	...
	protected void setCalendarForNextPeriodChange()
	{
		switch (_activePeriod)
		{
			...
			case RECRUITING, RESULTS -> _nextPeriodChange.add(Calendar.MILLISECOND, PERIOD_MINOR_LENGTH);
		}
	...
	public final int getCurrentScore(CabalType cabal)
	{
		...
		return switch (cabal)
		{
			case DAWN -> Math.round((float) (_dawnStoneScore / ((float) totalStoneScore == 0 ? 1 : totalStoneScore)) * 500) + _dawnFestivalScore;
			case DUSK -> Math.round((float) (_duskStoneScore / ((float) totalStoneScore == 0 ? 1 : totalStoneScore)) * 500) + _duskFestivalScore;
			default -> 0;
		};
	}
	...
	public final double getCurrentStoneScore(CabalType cabal)
	{
		return switch (cabal)
		{
			case DAWN -> _dawnStoneScore;
			case DUSK -> _duskStoneScore;
			default -> 0;
		};
	}
	...
	public final int getCurrentFestivalScore(CabalType cabal)
	{
		return switch (cabal)
		{
			case DAWN -> _dawnFestivalScore;
			case DUSK -> _duskFestivalScore;
			default -> 0;
		};
	}
	...
	public final int getSealProportion(SealType seal, CabalType cabal)
	{
		return switch (cabal)
		{
			case DAWN -> _dawnScores.get(seal);
			case DUSK -> _duskScores.get(seal);
			default -> 0;
		};
	}
	...
	public int addPlayerStoneContrib(int objectId, int blueCount, int greenCount, int redCount)
	{
		...
		switch (getPlayerCabal(objectId))
		{
			...
			case DUSK -> _duskStoneScore += contribScore;
		}
		...
		public void run()
		{
			...
			switch (periodEnded)
			{
				...
				case COMPETITION ->
				{
					...
					switch (winningCabal)
					{
						case DAWN -> World.toAllOnlinePlayers(SystemMessage.getSystemMessage(SystemMessageId.DAWN_WON));
						case DUSK -> World.toAllOnlinePlayers(SystemMessage.getSystemMessage(SystemMessageId.DUSK_WON));
					}
