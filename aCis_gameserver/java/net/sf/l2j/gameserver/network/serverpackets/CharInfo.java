...
public class CharInfo extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		writeC((_player.isHero() || (_player.isGM() && Config.GM_HERO_AURA) || (_player.isAIO() && Config.ALLOW_AIO_BUFFER_HERO_AURA)) ? 1 : 0);
		writeC((_player.isFishing()) ? 1 : 0);
		writeLoc(_player.getFishingStance().getLoc());
		writeD((_player.isAIO() && Config.ALLOW_AIO_BUFFER_COLOR_NAME) ? Integer.decode("0x" + Config.AIO_BUFFER_COLOR_NAME) : _player.getAppearance().getNameColor());
		writeD(_player.getHeading());
		writeD(_player.getPledgeClass());
		writeD(_player.getPledgeType());
		writeD((_player.isAIO() && Config.ALLOW_AIO_BUFFER_COLOR_TITLE) ? Integer.decode("0x" + Config.AIO_BUFFER_COLOR_TITLE) : _player.getAppearance().getTitleColor());
