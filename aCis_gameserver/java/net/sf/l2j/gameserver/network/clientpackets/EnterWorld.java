...
public class EnterWorld extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (player.isTitleColor() != 0)
			player.getAppearance().setTitleColor(player.isTitleColor());
       ...
       if (player.getMemos().getLong("aioEndTime", 0) > 0)
	       {
	           ...
	           final long endDay = player.getMemos().getLong("aioEndTime");
	           ...
	           {
	               player.setAIO(true);
	               sendReEnterMessage(endDay, player);
	           }           
	       }
	...
	private static void sendReEnterMessage(long time, Player player)
	{
	   ...
	   final int seconds = (int) ((remainingTime % 3600) % 60);
	    ...
	    String message = "Your AIO period ends after: %hours% hours, %mins% minutes and %secs% seconds!";
	    message = message.replaceAll("%hours%", Integer.toString(hours));
	    message = message.replaceAll("%mins%", Integer.toString(minutes));
	    message = message.replaceAll("%secs%", Integer.toString(seconds));
