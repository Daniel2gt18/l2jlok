...
public class AdminInfo implements IAdminCommandHandler
{
	...
	public void useAdminCommand(String command, Player player)
	{
		if (command.startsWith("admin_info"))
		{
			...
			{
				showDoorInfo((Door) targetWorldObject, html);
			}
	...
	private static void sendDropInfos(Npc npc, NpcHtmlMessage html, int page, boolean isDrop)
	{
		...
		for (DropData drop : list)
		{
			...
			StringUtil.append(sb, "<td width=246 height=34>", name, "<br1><font color=B09878>", ((isDrop) ? "Drop" : "Spoil"), ":</font> ", chance, "% <font color=B09878>Min:</font> ", drop.getMinDrop(), " <font color=B09878>Max:</font> ", drop.getMaxDrop(), " <font color=B09878>ID:</font> ", item.getItemId(), "</td>");
