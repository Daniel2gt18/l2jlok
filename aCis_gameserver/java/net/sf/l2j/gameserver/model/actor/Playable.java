...
public abstract class Playable extends Creature
{
	...
	public boolean canCastOffensiveSkillOnPlayable(Playable target, L2Skill skill, boolean isCtrlPressed)
	{
		...
		if (getActingPlayer().getDuelState() == DuelState.DUELLING && getActingPlayer().isInDuel() && targetPlayer.isInDuel() && getActingPlayer().getDuelId() == targetPlayer.getDuelId())
			return true;
		...
		if (aClan != null && tClan != null && aClan.isAtWarWith(tClan.getClanId()) && tClan.isAtWarWith(aClan.getClanId()) && !(isInsideZone(ZoneId.CASTLE) && targetPlayer.isInsideZone(ZoneId.CASTLE) && isInsideZone(ZoneId.PVP) && targetPlayer.isInsideZone(ZoneId.PVP)))
			return isCtrlPressed;
		else if (aClan != null && tClan != null && aClan.isAtWarWith(tClan.getClanId()) && tClan.isAtWarWith(aClan.getClanId()) && (isInsideZone(ZoneId.CASTLE) && targetPlayer.isInsideZone(ZoneId.CASTLE) && isInsideZone(ZoneId.PVP) && targetPlayer.isInsideZone(ZoneId.PVP)))
			return true;
