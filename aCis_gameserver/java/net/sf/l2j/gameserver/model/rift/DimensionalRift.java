...
public class DimensionalRift
{
	...
	protected void chooseRoomAndTeleportPlayers(byte type, List<Player> players, boolean canUseBossRoom)
	{
		...
		final List<DimensionalRiftRoom> list = DimensionalRiftManager.getInstance().getFreeRooms(type).stream().filter(r -> !_completedRooms.contains(r.getId())).collect(Collectors.toList());
